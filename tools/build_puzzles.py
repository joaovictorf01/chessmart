...
def build_tier(csv_path: Path, tier: str, out_dir: Path, source_info: dict) -> dict:
    ...
    log(f"  pronto: {entry['bytes'] / 1e6:.0f} MB no disco, {entry['download']['bytes'] / 1e6:.0f} MB para baixar")
    return entry
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--cache-dir", type=Path, default=Path("build/lichess"), help="onde o download fica guardado")
