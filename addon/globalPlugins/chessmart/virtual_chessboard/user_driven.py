...
class UserDrivenChessboard(BaseVirtualChessboard):
	...
	def is_dragable(self, index):
		...
		return (piece is not None) and (piece.color is self.prospective) and (not self.is_busy(index))
	...
	def make_drop_move(self, index, piece_type):
		move = chess.Move(from_square=index, to_square=index, drop=piece_type)
		self.move_piece_and_check_game_status(move)
	...
	def make_opponent_draw_offer(self):
		...
		self._current_focused_object = DrawChoiceMenu(
			...
			name=_("{color} is offering to draw. Do you want to accept the draw offer?").format(
				color=color_name,
			),
			parent=self,
		)
...
class PieceSelectionMenu(MenuObject):
	...
	def event_gainFocus(self):
		super().event_gainFocus()
		queueHandler.queueFunction(queueHandler.eventQueue, GameSound.request_promotion.play)
