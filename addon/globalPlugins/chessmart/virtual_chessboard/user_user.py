...
class UserUserChessboard(UserDrivenChessboard):
	...
	def move_piece_and_check_game_status(self, move, pre_speech=(), post_speech=()):
		...
		if self.board.turn is self.prospective:
			post_speech.append(
				speech.commands.CallbackCommand(lambda: eventHandler.queueEvent("gainFocus", self)),
			)
		return super().move_piece_and_check_game_status(move, pre_speech, post_speech=post_speech)
