...
class SimpleList(MenuObject):
	...
	def event_gainFocus(self):
		...
		queueHandler.queueFunction(queueHandler.eventQueue, GameSound.score_list_open.play)

	def close_menu(self):
		super().close_menu()
		queueHandler.queueFunction(queueHandler.eventQueue, GameSound.score_list_close.play)
	...
	def add_item(self, item_text):
		self.items.insert(
			0,
			SimpleListItem(parent=self, name=item_text, close_gesture=self.close_gesture),
		)
