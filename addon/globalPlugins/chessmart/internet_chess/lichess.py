...
from .abstract.events import InternetChessBoardEvent
...
class LichessAPIClient(InternetChessAPIClient):
	...
	async def seek_game(
		...
		await self.connect()
		base_time, increment = self._get_time_control_info(self.game_info.time_control)
		await self.lichess.boards.create_a_seek(
			time=base_time / 60,
			increment=increment,
			rated=rated,
			color=self._requested_color(),
...
class LichessBoardClient(InternetChessBoardClient):
	...
	async def start_realtime_game_stream(self):
		...
		async for response in self.client.lichess.boards.stream_game_state(game_id=self.game_id):
	...
	async def send_chat_message(self, message):
		return await self.lichess.boards.write_in_chat(
			game_id=self.game_id,
			message=message,
			room=RoomTypes.PLAYER,
		)

	@asyncio_coroutine_to_concurrent_future
	async def send_move(self, move, draw=False):
		return await self.lichess.boards.make_move(game_id=self.game_id, move=move.uci(), draw=draw)
	...
	def _process_game_state_started(self, status, maybe_restore_state=False):
		...
		event = InternetChessBoardEvent.move_made(chess.Move.from_uci(moves[-1]), player=move_turn)
	...
	def _handle_realtime_game_stream_status(self, data):
		...
		if status_type == "gameState":
			if status["status"] == "started":
				...
				time_forfeit_event = InternetChessBoardEvent.game_time_forfeit(loser=loser)
			...
			time_control = ChessTimeControl.from_string(f"{int(base_time)};{int(increment)}")
			info = InternetGameInfo(
				white_username=status["white"]["name"],
				black_username=status["black"]["name"],
				time_control=time_control,
				user_color=chess.WHITE if status["white"]["name"] == self.username else chess.BLACK,
