...
class PuzzleDownloadDialog(wx.Dialog):
    ...
    def _build(self):
        ...
        self.tierRadio = wx.RadioBox(
            self, -1, _("Database to download"), choices=[TIER_LABELS[tier] for tier in TIER_ORDER],
    ...
    def _installed_sentence(self) -> str:
        ...
        return _("Installed: {tier}, {count:,} puzzles, Lichess base of {when}.").format(
            tier=label, count=self.installed.puzzle_count, when=when
        )
        ...
        for index, tier in enumerate(TIER_ORDER):
            ...
            self.tierRadio.SetItemLabel(
                ...
                _("{name}: {count:,} puzzles, {download} to download, {disk} on disk").format(
                    name=TIER_LABELS[tier], count=info.puzzle_count,
                    download=_megabytes(info.download_bytes), disk=_megabytes(info.disk_bytes),
                ),
            )
    ...
    def onDownload(self, event):
        ...
        ui.message(_("Downloading {name}, {size}.").format(name=TIER_LABELS[info.tier], size=_megabytes(info.download_bytes)))

        def work():
            try:
                puzzle_download.download_tier(info, self.target_path, progress=self._progress, cancel=self._cancel)
    ...
    def _show_progress(self, done, total, percent):
        ...
        self.progressText.SetLabel(
            ...
            _("{done} of {total} ({percent}%)").format(done=_megabytes(done), total=_megabytes(total), percent=percent)
        )
    ...
    def _finished_ok(self, info):
        ...
        message = _("The {name} puzzle database is installed: {count:,} puzzles.").format(
            name=TIER_LABELS[info.tier], count=info.puzzle_count
        )
