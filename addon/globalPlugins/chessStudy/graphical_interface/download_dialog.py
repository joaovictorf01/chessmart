...
class PuzzleDownloadDialog(DownloadDialogBase):
	...
	def _start_manifest_fetch(self):
		def work():
			try:
				...
				log.warning("chessStudy: could not fetch the puzzle manifest: %s", error)
				wx.CallAfter(self._manifest_failed, str(error))
				return
			wx.CallAfter(self._manifest_ready, manifest)

		threading.Thread(target=work, name="chessStudy.manifest", daemon=True).start()
	...
	def onDownload(self, event):
		...
		def work():
			try:
				...
				log.error("chessStudy: puzzle database download failed: %s", error)
				wx.CallAfter(self._finished_failed, str(error))
				return
			wx.CallAfter(self._finished_ok, info)

		self.start_worker(work, "chessStudy.download")
	...
	def _retrying(self, file: str, attempt: int, attempts: int, reason: str):
		...
		log.warning(
			"chessStudy: %s failed (attempt %d of %d): %s; downloading it again",
			file,
			attempt,
			attempts,
			reason,
	...
	def _rebuild_theme_catalog(self):
		...
		try:
			...
			log.warning("chessStudy: theme catalog rebuild after download failed: %s", error)
