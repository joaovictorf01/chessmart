...
class TablebaseJudgeMixin(ActionsBarMixin):
	...
	def open_judge(self):
		try:
			...
			nvda_log.warning("chessStudy: could not open the tablebases: %s", error)
			self.tablebase = None
	...
	def close_judge(self):
		if self.tablebase is not None:
			try:
				...
				nvda_log.debug("chessStudy: tablebase close failed", exc_info=True)
			self.tablebase = None
...
def _record(lesson_id: str, position_id: str, **fields) -> None:
	...
	try:
		...
		nvda_log.warning("chessStudy: could not record the endgame attempt: %s", error)
