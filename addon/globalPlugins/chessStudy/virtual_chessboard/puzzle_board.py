	...
	def script_next_puzzle(self, gesture):
		...
		if puzzle is not None and self.parent.current_expected_move is not None:
			if getLastScriptRepeatCount() == 0:
				...
				return
			log.info("chessStudy: tactic %s skipped with Control+N", puzzle.puzzle_id)
...
class PuzzleChessboard(ActionsBarMixin, UserDrivenChessboard):
	...
	def _on_skip_reviews_answer(self, skip: bool):
		if not self.dialog:
			return
		if skip:
			log.info("chessStudy: reviews skipped, %d left", self.session.reviews_left + 1)
	...
	def report_rating(self):
		...
		try:
			...
			log.exception("chessStudy: failed to read the rating")
			summary = None
		if summary is None:
	...
	def _write_attempt(self, solved: bool):
		...
		try:
			...
			log.exception("chessStudy: failed to record the attempt")
	...
	def _write_review(self, solved: bool):
		...
		try:
			...
			log.exception("chessStudy: failed to record the review")
