...
class ChessboardMenu(wx.Menu):
	...
	def onRandomPuzzle(self, event):
		...
		self._open_session(
			...
			_("The tactics database was not found. Choose a valid database in Chess Study settings first."),
		)
	...
	def _random_drill_fen(self, drill) -> str:
		...
		try:
			...
			log.warning("chessStudy: could not open the tablebase for the drill draw: %s", error)
			tables = None
	...
	def onMyGames(self, event):
		...
		for name in unreadable:
			log.warning("chessStudy: My Games could not read %s", name)
		if not games:
			show_message(
	...
	def _on_analysis_file_chosen(self, dialog, res):
		...
		try:
			...
			log.warning("chessStudy: could not read PGN file %s: %s", filepath, error)
	...
	def _open_game_for_analysis(self, game_info, single_game_file):
		try:
			...
			log.warning("chessStudy: could not read PGN game %s: %s", game_info.description, error)
			self._say_pgn_unreadable(error)
			return
	...
	def _on_lichess_game(self, future):
		try:
			...
			log.warning("chessStudy: Lichess import failed: %s", error)
			show_error(
		...
		if os.path.exists(path):
			...
			try:
				...
				log.warning("chessStudy: could not read %s: %s", path, error)
				self._say_pgn_unreadable(error)
				return
...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
	...
	# -- shortcuts ----------------------------------------------------------------
	# The menu lives under NVDA > Tools > Chess Study; daily training shouldn't
	# require three levels of menu. Only Tactics has a default key; the
	# others the user enables in Input Gestures, under the Chess Study category.
	...
	@script(
		...
		description=_("Starts a random Chess Study puzzle with the saved training setup"),
	)
	...
	@script(
		...
		description=_("Opens Chess Study endgames"),
	)
	...
	@script(
		...
		description=_("Opens Chess Study My Study"),
	)
	...
	@script(
		...
		description=_("Starts a new Chess Study game"),
	)
	...
	@script(
		...
		description=_("Opens Chess Study My Games"),
	)
	...
	def terminate(self):
		...
		for cdlg in list(self._active_board_dialogs.values()):
			try:
				...
				log.exception("chessStudy: could not close a board at exit")
		try:
